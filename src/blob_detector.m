function label_map = blob_detector(binary_map)
    %--------------------------------------------------------------
    % detect the main blob of an image, eliminate background noise
    % reference: en.wikipedia.org/wiki/Connected-component_labeling
    %--------------------------------------------------------------
...
%     label_map = zeros(row, col);
    ...
    if((~addRowStart && max(y) == row) || (addRowStart && max(y) == row - 1))
        row = row + 1;
        binary_map(row, :) = 0;
        addRowEnd = 1;
    end
    ...
    if((~addColStart && max(x) == col) || (addColStart && max(x) == col - 1))
        col = col + 1;
